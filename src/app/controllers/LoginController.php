    ...
    protected function _doLogin()
    {
        ...
        if (!$result->isValid()) {
            ...
            if ($config->$authAdapterMethod->createUserInDBOnLogin) {
                ...
                if (in_array("createUserInDB", get_class_methods($authAdapter)) && $authAdapter->createUserInDB()) {
                    if (!$user) {
                        $data = array(
                        ...
                        $user->save();
                    //Update password on usvn database with current password
                    } else {
                        $user['users_password'] = $_POST['password'];
                        $user->save();
                    }
